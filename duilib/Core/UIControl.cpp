...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if( _tcscmp(pstrName, _T("pos")) == 0 ) {
        ...
        // Jeffery (2013/12/22)
        // 扩展pos属性的功能，加入pos="5,9,=18,=18"格式。
        // 只有right，bottom支持=, =后面的值为宽或高。
        // 如 pos="5,9,=18,=18" 代表left = 5, top = 9, right = 5+18 = 27, bottom = 9+18 = 27
        const TCHAR * pstrSplit = _T(",");
        TCHAR * pstrRes = NULL;
        int i = 0;
        pstrRes = _tcstok((TCHAR*)pstrValue, pstrSplit);

        while(pstrRes) {
            if(0 == i) { // left
                rcPos.left = _tstoi(pstrRes);
            } else if(1 == i) { // top
                rcPos.top = _tstoi(pstrRes);
            } else if(2 == i) { // right
                if(_tcsstr(pstrRes, _T("="))) {
                    pstrRes += 1;
                    rcPos.right = rcPos.left + _tstoi(pstrRes);
                } else {
                    rcPos.right = _tstoi(pstrRes);
                }
            } else if(3 == i) { // bottom
                if(_tcsstr(pstrRes, _T("="))) {
                    pstrRes += 1;
                    rcPos.bottom = rcPos.top + _tstoi(pstrRes);
                } else {
                    rcPos.bottom = _tstoi(pstrRes);
                }
            }

            i++;
            pstrRes = _tcstok(NULL, pstrSplit);
        }
